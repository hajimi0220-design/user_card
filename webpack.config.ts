...
const copy_dist_files = [
  ...
  { src: 'dist/角色卡/界面/状态栏/theme-config.js', dest: 'dist/角色卡/界面/状态栏/theme-config.js' },
  { src: 'dist/角色卡/界面/状态栏/README.md', dest: 'dist/角色卡/界面/状态栏/README.md' },
];
const copy_static_files = [
  { src: 'src/角色卡/界面/状态栏/theme-config.js', dest: 'dist/角色卡/界面/状态栏/theme-config.js' },
  { src: 'src/角色卡/界面/状态栏/README.md', dest: 'dist/角色卡/界面/状态栏/README.md' },
];
function copy_dist(compiler: webpack.Compiler) {
  compiler.hooks.done.tap('copy_dist', () => {
    for (const { src, dest } of copy_static_files) {
      const src_path = path.join(import.meta.dirname, src);
      const dest_path = path.join(import.meta.dirname, dest);
      if (fs.existsSync(src_path)) {
        fs.mkdirSync(path.dirname(dest_path), { recursive: true });
        fs.copyFileSync(src_path, dest_path);
      }
    }
    for (const { src, dest } of copy_dist_files) {
      const src_path = path.join(import.meta.dirname, src);
      const dest_path = path.join(SILLY_TAVERN_PUBLIC, dest);
      if (fs.existsSync(src_path)) {
        fs.mkdirSync(path.dirname(dest_path), { recursive: true });
        fs.copyFileSync(src_path, dest_path);
      }
    }
    console.info('\x1b[36m[copy_dist]\x1b[0m 已复制 dist 文件到 SillyTavern public 目录');
  });
}
